...
number = st.number_input(label="Choose a row", min_value=0, max_value=40000, step=1)
...
""")
one_hot_encoded = encoder.transform(app_test[categorical])
one_hot_df = pd.DataFrame(one_hot_encoded, columns=encoder.get_feature_names_out(categorical)).set_index([pd.Index([number])])
# print(one_hot_df.shape)
# st.write(one_hot_df.shape)
# st.dataframe(one_hot_df)
# print(app_test.drop(categorical, axis=1).shape)
# st.write(app_test.drop(categorical, axis=1).shape)
# st.dataframe(app_test.drop(categorical, axis=1))
app_test = pd.concat([app_test.drop(categorical, axis=1), one_hot_df], axis=1)
print(app_test.shape)
st.write(app_test.shape)
st.dataframe(app_test)
...
print(f'Probability of default is {round(100 * res[0], 1)}%')
""")
...
bst = lgb.Booster(model_file='model.txt')
res = bst.predict(domain_features_test)
st.write(f'Probability of default is {round(100 * res[0], 1)}%')
print(f'Probability of default is {round(100 * res[0], 1)}%')
